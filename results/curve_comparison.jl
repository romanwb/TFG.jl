...
for i in eachindex(dataset)
    paths[i] = "scripts/realistic_models/turbine_blade_model/study_of_ext_force/data/EO1/$(dataset[i])_ff1.jld2"
end
...
begin
    ...
    ax2 = Axis(fig[1, 2])
        ...
        for i in eachindex(paths)
            ...
            # lines!(ax2, (p[i].ω₀²/ω₀²real) * Ω[i], (p[i].α/αreal) * a_max[i]; label="$(dataset[i])")
            lines!(ax2, (ω₀²real/p[i].ω₀²) * Ω[i], (αreal/p[i].α) * a_max[i]; label="$(dataset[i])")
        end
