using HBMContinuation, GLMakie, LinearAlgebra, Julianim, TFG, JLD2, SparseArrays, ProgressMeter
    ...
    dataset=("345x345", "23456x23456", "34x34", "1-7x1-7", "first_last_cols", "first_last_rows", "first2colums", "upper2rows")
    EOs=[1, 5, 10, 20, 30, 60]
    key = 8
    ...
    @load "data/tbm/ns146_raw/ns146_$(dataset[key]).jld2" contenido
        ...
        Maxᵏ_full_nat, Mxaᵏ_full_nat, Mxxᵏ_full_nat, Kxxᵏ_full_nat, fₐᵏ_full_nat, fₓᵏ_full_nat, ωₐ²ᵏ_full_nat, Φᵏ_nat, Ψᵏ_nat, Rₓ_nat = Maxᵏ_full, Mxaᵏ_full, Mxxᵏ_full, Kxxᵏ_full, fₐᵏ_full, fₓᵏ_full, ωₐ²ᵏ_full, Φᵏ, Ψᵏ, Rₓ

for m in eachindex(EOs)
    Maxᵏ_full, Mxaᵏ_full, Mxxᵏ_full, Kxxᵏ_full, fₐᵏ_full, fₓᵏ_full, ωₐ²ᵏ_full, Φᵏ, Ψᵏ, Rₓ = Maxᵏ_full_nat, Mxaᵏ_full_nat, Mxxᵏ_full_nat, Kxxᵏ_full_nat, fₐᵏ_full_nat, fₓᵏ_full_nat, ωₐ²ᵏ_full_nat, Φᵏ_nat, Ψᵏ_nat, Rₓ_nat
...
for i in 1:Int(length(Rₓ))
    ...
    begin
        ...
        kₙ, kₜ₁, kₜ₂, μ = 1e9*kr, 1e6*kr, 1e6*kr, 0.1
...
p = (
 ...
 @save "scripts/tbm_cyclic/1.first_section/5_different_dataset/data/EO$(EO)/$(dataset[key]).jld2" Ω Xⱼ Aⱼ p
end
...
# H = 3
     @load "scripts/tbm_cyclic/1.first_section/4. Different EO/data/EO1/7x7.jld2" Ω Xⱼ Aⱼ p
H = p.H
   X₁ = Xⱼ[1:(2H + 1), :]
    # X₁ = Xⱼ[40*(2H + 1) + 1:41*(2H + 1), :]
    # X₁ = Xⱼ[(2H + 2):(4H + 2), :]
    # X₁ = Xⱼ[(4H + 3):(6H + 3), :]
    E, Eᴴ = HBMContinuation.dft_matrices(2^14, H)
    x₁ = E * X₁
    ...
    a_max = vec(maximum(abs.(a₁), dims = 1))
        ...
        # ax1 = Axis(fig[1, 1], limits = (0.89, 0.95, nothing, nothing), xlabel = "Ω", ylabel = L"\text{max}|a_{1}|")

            lines(Ω, a_max; label = "H = $(H)")
...
fig
using CairoMakie
save("scripts/tbm/3.turbine_blade_model/1.first_section/Hs.pdf", fig; backend=CairoMakie)
