...
for i in eachindex(dataset)
    paths[i] = "scripts/tbm_cyclic/1.first_section/5_different_dataset/data/EO1/$(dataset[i]).jld2"
end
...
for i in eachindex(paths)
    ...
    lines!(ax1, p[i].ω₀² * Ω[i], p[i].α * a_max[i]; label="$(dataset[i])")
end
...
limits!(1.25e7, 2.0e7, 0, nothing)
fig
     using CairoMakie
     save("scripts/tbm_cyclic/1.first_section/5_different_dataset/dataset_comparison.pdf", fig; backend=CairoMakie)
