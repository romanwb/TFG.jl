...
# @showprogress 1 "Procesando..."  for m in eachindex(EOs)
    ...
    force_values = [0.5, 1, 2, 4]

for n in eachindex(force_values)
...
for i in 1:Int(length(Rₓ)/3)
    println("coefficient: ", 3i, "value: ", Rₓ[3i])
end
...
begin
    ...
    force_name = ["5e-1", "1", "2", "4"]
     @save "scripts/tbm_cyclic/study_of_ext_force/data/EO$(EO)/$(dataset[key])_ff$(force_name[n]).jld2" Ω Xⱼ Aⱼ p
end
# end
