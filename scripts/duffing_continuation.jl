...
cont_pars = ContinuationParameters(λmin = 0.0, λmax = 2.0, Δs = 0.1, maxsteps = 3000,
...
amplitudes_abs = [maximum(E * abs.(sol.x[:, i])) for i in 1:length(λ_values)]
...
fig = Figure()
plot_comparison(amplitudes, 1)
plot_comparison(amplitudes_abs, 2)
