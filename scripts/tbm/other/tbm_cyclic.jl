...
begin
    ...
    ContPars = ContinuationParameters(
        λmin = Ω₀, λmax = Ωₘₐₓ, Δs = 0.01/EO, maxsteps = 10_000,
    ...
    lines(Ω, a_max)
    limits! = (0.92, 1.0, 0, 100)
    begin
    f = Figure()
    ax1 = Axis(f[1, 1], xlabel = "x label", ylabel = "y label",
    ...
    lines!(ax1, Ω, a_max)
    lines!(ax2, Ω * ω₀², a_max * α)
    lines!(ax3, Ω, x_max)
    lines!(ax4, Ω * ω₀², x_max * α)
    f
    end
    ...
    # force_name = ["2", "4"]
     @save "scripts/realistic_models/turbine_blade_model/study_of_ext_force/data/EO$(EO)/$(dataset[key])_ff$(force_name[n]).jld2" Ω Xⱼ Aⱼ p
