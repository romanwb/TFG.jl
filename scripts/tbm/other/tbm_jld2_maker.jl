using HBMContinuation, GLMakie, LinearAlgebra, Julianim, TFG, JLD2, SparseArrays
    ...
    ω² = Vector{Float64}
    Max = Matrix{ComplexF64}
    Mxx = Matrix{ComplexF64}
    Kxx = Matrix{ComplexF64}
    fₐ  = Vector{ComplexF64}
    fₓ  = Vector{ComplexF64}
    Φ   = Matrix{ComplexF64}
    Ψ   = Matrix{ComplexF64}
    N = 146

    _, Max, Mxx, _, Kxx, ω², fₐ, fₓ, Φ, Ψ, _ = craig_bampton_aplication(Mee, Mec, Mcc, Kee, Kec, Kcc, fe, fc; n_modes=5)

     @save "data/tbm/ns146raw_eo0/7x7_eo0_processed.jld2" Max Mxx Kxx fₐ fₓ ω² Φ Ψ Rx
